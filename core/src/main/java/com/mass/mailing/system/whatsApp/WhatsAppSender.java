...
public class WhatsAppSender extends Messager {
    ...
    public static final String USER_PHONE = "<phone>"; //type here registered phone number to send messages from

    //to get password use "python yowsup-cli demos --help-config"
    public static final String PASSWORD = "<password>"; //type here password in base64
    ...
    public void execute() {
        ...
        ContactSerializer.serialize(getMessageService().getContacts(), getMessageService().getMessageTemplate());

        try {
            //for python script successful, run terminal command must look like that:
            //python run.py <textFileName> <userPhone> <password base64>
